#!/usr/bin/env bash
# Shared paths for ocpai-prd-mtz overlay scripts.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
CLUSTER="${CLUSTER:-${ROOT}/clusters/ocpai-prd-mtz}"
...
warn_if_wrong_cluster() {
  ...
  if [[ "${ctx}" != *ocpai-prd-mtz* && "${ctx}" != *prd-mtz* ]]; then
    echo "WARNING: current context '${ctx}' does not look like ocpai-prd-mtz." >&2
